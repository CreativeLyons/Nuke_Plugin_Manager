...
"""
        ...
        # Quote python_cmd if it contains spaces
        python_cmd_quoted = f'"{python_cmd}"' if ' ' in python_cmd else python_cmd

        # Create executable script that uses run_panel (bootstrap + venv)
...
cd "{installable_root}"
nohup {python_cmd_quoted} core/run_panel.py >/dev/null 2>&1 &
"""
...
def _create_windows_launcher(installable_root: Path, plugin_manager_dir: Path, python_cmd: str) -> bool:
    ...
    try:
        # Convert paths to Windows format (backslashes) and quote if needed
        installable_root_win = str(installable_root).replace('/', '\\')
        # Quote python_cmd if it's a full path (contains backslash) or has spaces
        python_cmd_quoted = f'"{python_cmd}"' if ('\\' in python_cmd or ' ' in python_cmd) else python_cmd
...
cd /d "{installable_root_win}"
start "" {python_cmd_quoted} -c "import sys; sys.path.insert(0, 'core'); from app import main; main()"
"""
...
cd "{installable_root}"
{python_cmd_quoted} -c "import sys; sys.path.insert(0, 'core'); from app import main; main()" &
"""
        ...
        # Create .desktop file - quote script path in case it contains spaces
        script_path_str = str(script_path)
        exec_path = f'"{script_path_str}"' if ' ' in script_path_str else script_path_str
